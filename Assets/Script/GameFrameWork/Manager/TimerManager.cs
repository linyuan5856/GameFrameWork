...
public class GlobalTimer : MonoBehaviour
{
    class TimeData
    {
        ...
        public void Update(float start, double end)
        {
    ...
    public static GlobalTimer Instance
    {
        get
        {
            if (_instance == null)
            {
                UnityEngine.Debug.LogError("全局计时器 初始化未完成 请稍后调用 ");
            }
            return _instance;
        }
    }
    ...
    void Update()
    {       
        foreach (var data in this._timerDic.Values)
        {
    ...
    public void RegisterTimer(int type, double endTime,int interval=1)
    {
        ...
        {
            this._timerDic[type].Update(startTime, endTime);       
        }
