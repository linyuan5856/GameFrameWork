...
namespace Pandora
{
    ...
    public class Tool_PrefabLightmapData : MonoBehaviour
    {
        ...
        const string LIGHTMAP_RESOURCE_PATH = "Assets/Sources/PrefabLightmaps/";
        ...
        public static void GenerateLightmapInfo()
        {
            ...
            var sceneName = Path.GetFileNameWithoutExtension(SceneManager.GetActiveScene().path);
            var resourcePath = LIGHTMAP_RESOURCE_PATH + sceneName;
            var scenePath = System.IO.Path.GetDirectoryName(SceneManager.GetActiveScene().path) + "/" + sceneName + "/";
