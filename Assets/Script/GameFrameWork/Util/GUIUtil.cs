...
public class GUIUtil {
    ...
    private static Color old_color = GUI.color;
#if UNITY_EDITOR
    public static void DrawRect(Rect rect, Color color, float alpha = -1)
    {
        GUIUtil.SetColor(color, alpha);
        GUI.DrawTexture(rect, UnityEditor.EditorGUIUtility.whiteTexture);
        GUIUtil.RestoreColor();
    }
#endif
    public static Rect ScaleRect(Rect rect, float scaleX, float scaleY = 0)
    {
        if (scaleX <= 0)
            scaleX = 1;
        ...
        float new_width = rect.width * scaleX;
        float new_height = rect.height * scaleY;
        float new_left = rect.x + ((rect.width - new_width) / 2);
        float new_top = rect.y + ((rect.height - new_height) / 2);

        return new Rect(new_left, new_top, new_width, new_height);
    }

    public static Rect GetRectCenter(Rect rect, float width, float height)
    {
        if (width <= 0)
            width = 1;
        ...
        float new_left = rect.x + ((rect.width - width) / 2);
        float new_top = rect.y + ((rect.height - height) / 2);

        return new Rect(new_left, new_top, width, height);
    }
