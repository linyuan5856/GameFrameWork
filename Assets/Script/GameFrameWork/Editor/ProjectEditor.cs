...
using Logger = GFW.Logger;

public class ProjectEditor
{

    public const string Path_GenerateExcel = "Assets/_Generate/_Excel";
    public const string Path_EditorConfig = "Assets\\Editor\\Config\\EditorConfig.asset";

    [MenuItem("program/Config/GenerateConfig", false, 0)]
    private static void GenerateConfig()
    {
        EditorConfig editorConfig = AssetDatabase.LoadAssetAtPath<EditorConfig>(Path_EditorConfig);
        if (editorConfig == null)
        {
            editorConfig = new EditorConfig();
            AssetDatabase.CreateAsset(editorConfig, Path_EditorConfig);
        }
        Selection.activeObject = editorConfig;
    ...
    private static void BuildAllTable()
    {
        ...
        for (int i = 0; i < EditorConfig.Instance.tableList.Count; i++)
        {
            tableConfig = EditorConfig.Instance.tableList[i];
            if (!string.IsNullOrEmpty(tableConfig.rowClassName))
            {
                BuildOneTable(tableConfig.tableClassName, tableConfig.rowClassName, EditorConfig.Instance.Path_ExcelInputDir + tableConfig.shortPath);
            }
            else
            {
                BuildOneTable(tableConfig.tableClassName, EditorConfig.Instance.Path_ExcelInputDir + tableConfig.shortPath);
            }
        }
    ...
    //建立MySql数据库连接
    static public MySqlConnection openConn()
    {
    ...
    //执行MySqlCommand命令
    static public void ExcecuteCMD(MySqlConnection con, string M_str_sqlstr)
    {
    ...
    static public void CloseConn(MySqlConnection mysqlcon)
    {
        mysqlcon.Close();
        mysqlcon.Dispose();
    }
    ...
    private static void BuildOneTable()
    {
        ...
        string path = EditorUtility.OpenFilePanel("Select Excel", EditorConfig.Instance.Path_ExcelInputDir, "xlsx");
        path = path.Replace("\\", "/");
        if (string.IsNullOrEmpty(path)) return;
        TableConfig tableConfig = null;
        for (int i = 0; i < EditorConfig.Instance.tableList.Count; i++)
        {
            string shortPath = EditorConfig.Instance.tableList[i].shortPath;
    ...
    private static void BuildOneTable(string tableTypeName, string rowTypeName, string path)
    {
        ...
        //Type tableType = Type.GetType(tableTypeName);
        //Type rowType = Type.GetType(rowTypeName);
    ...
    private static void BuildAllServerTable()
    {
        ...
        try
        {
            Logger.Log(EditorConfig.Instance.Path_ExcelInputDir);
            List<string> list = new List<string>();
            GetAllFile(EditorConfig.Instance.Path_ExcelInputDir, list);
    ...
    [MenuItem("program/ConfigBuildOneServerTable", false, 0)]
